...
- 提供 macOS、Windows、Linux 预构建安装包 / 压缩包；也可以从源码自行构建。
...
- **跨平台使用**：提供 macOS `.dmg/.zip`、Windows `.exe`、Linux `.deb` / `.AppImage` / 便携 `.tar.gz`；也可以从源码构建。
...
| Linux | `.deb`（Debian / Ubuntu 优先）、`.AppImage` 或便携 `.tar.gz`，x64 |
...
- 如果你安装的是 Debian 系 `.deb`，可用 `sudo apt remove deepseek-gui` 或 `sudo dpkg -r deepseek-gui` 卸载。
- 如果你使用的是 `.AppImage` 或便携 `.tar.gz`，删除对应下载文件或解压目录即可。
...
npm run dist:linux      # Linux DEB + AppImage + 便携 tar.gz
npm run dist:linux:deb  # Linux DEB（第一优先）
